...
class TorchGeneratorAgent(TorchAgent):
    ...
    def upgrade_opt(cls, opt_from_disk):
        ...
        if 'inference' not in opt_from_disk:
            ...
            if opt_from_disk['beam_size'] == 1:
                opt_from_disk['inference'] = 'greedy'
            else:
                opt_from_disk['inference'] = 'beam'
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument(
            '--inference',
            choices={'beam', 'greedy', 'topk', 'nucleus', 'topp'},
            default='greedy',
            help='Generation algorithm',
        )
        agent.add_argument(
            '--topk', type=int, default=2, help='K used in Top K sampling'
        )
        agent.add_argument(
            '--topp', type=float, default=0.3, help='p used in nucleus sampling'
        )
    ...
    def _treesearch_factory(self, device):
        ...
        elif method == 'nucleus' or method == 'topp':
...
class NucleusSampling(TreeSearch):
    ...
    Samples from a truncated distribution which covers a fixed CDF porition
